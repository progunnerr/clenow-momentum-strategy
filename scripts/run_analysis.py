...
from clenow_momentum.indicators.risk import build_portfolio
...
from clenow_momentum.utils.config import (
    get_position_sizing_guide,
    load_config,
    validate_config,
...
def print_account_sizing(config):
    ...
    sizing_guide = get_position_sizing_guide(
        config["strategy_allocation"], config["risk_per_trade"]
    )
...
def check_and_filter_stocks(momentum_df, stock_data, config, trading_allowed):
    ...
    if trading_allowed:
        ...
        print(
            f"📈 Sizing {len(stocks_for_portfolio)} filtered stocks to fill up to {config['max_positions']} positions."
        )
...
def display_portfolio_table(portfolio_df, config):
    ...
    for _, row in portfolio_df.iterrows():
        atr_impact = row.get("actual_atr_impact", row["shares"] * row["atr"])
        stop_risk = row.get("stop_loss_risk", row.get("actual_risk", 0))
        table_data.append(
            [
    ...
    if "actual_atr_impact" in portfolio_df:
        total_atr_impact = portfolio_df["actual_atr_impact"].sum()
    else:
        total_atr_impact = (portfolio_df["shares"] * portfolio_df["atr"]).sum()
    if "stop_loss_risk" in portfolio_df:
        total_stop_risk = portfolio_df["stop_loss_risk"].sum()
    else:
        total_stop_risk = portfolio_df["actual_risk"].sum()
    ...
    print(f"📊 Total ATR Impact: ${total_atr_impact:,.0f}")
    print(
        f"⚠️  Total {config['stop_loss_multiplier']:g}x ATR Stop Exposure: ${total_stop_risk:,.0f}"
    )
    print(
        f"🎯 Target ATR Impact per Position: {config['risk_per_trade']:.3%} (${config['strategy_allocation'] * config['risk_per_trade']:,.0f})"
    )
    print(f"📏 Stop Multiplier: {config['stop_loss_multiplier']:g}x ATR")
    print("📊 Weighting Method: Risk Parity (Equal ATR Impact)")
    display_dropped_candidates(portfolio_df)


def display_dropped_candidates(portfolio_df):
    """Display candidates dropped during portfolio construction."""
    drop_reasons = portfolio_df.attrs.get("drop_reasons", [])
    if not drop_reasons:
        return
    ...
    print(tabulate(table_data, headers=["Ticker", "Stage", "Reason"], tablefmt="grid"))
...
def main(force_execution: bool = False):
    ...
    market_regime = {
        ...
        "price_vs_ma": regime.price_vs_ma_pct / 100
        if regime.price_vs_ma_pct
        else None,  # Convert pct to decimal
    ...
    if not has_error(ma_analysis):
        ...
        struct_emoji = (
            ...
            else ("📈" if "Bullish" in structure else "⚖️" if "Mixed" in structure else "📉")
        )
        ...
        if ma_analysis.get("mas_aligned"):
            ...
            print(f"   ⚠️ MA stack not bullishly aligned (target: {MA_ALIGNMENT_TARGET})")
            if alignment_stack:
    ...
    if not has_error(momentum_data):
        ...
        print(
            f"{momentum_emoji} {universe_spec.display_name} 12-Month Return: {period_return:+.2f}%"
        )
            ...
            try:
                ...
                if critical_issues:
                    ...
                    try:
                        ...
                        print(
                            f"✅ Portfolio synced: {current_portfolio.num_positions} positions, ${current_portfolio.cash:,.0f} cash"
                        )
